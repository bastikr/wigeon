...
TEST(AREA2D, OPERATORS) {
  ...
  ASSERT_DOUBLE_EQ(boost::get<Circle2D>(area_p.exterior_curves.front()).origin().x(), 3);
  ASSERT_DOUBLE_EQ(boost::get<Circle2D>(area_p.exterior_curves.front()).origin().y(), 6);

  ASSERT_DOUBLE_EQ(boost::get<Circle2D>(area_m.exterior_curves.front()).origin().x(), 1);
  ASSERT_DOUBLE_EQ(boost::get<Circle2D>(area_m.exterior_curves.front()).origin().y(), 2);
...
TEST(AREA2D, ROTATE) {
  ...
  ASSERT_DOUBLE_EQ(boost::get<Circle2D>(area_rotated.exterior_curves.front()).origin().x(), -4);
  ASSERT_DOUBLE_EQ(boost::get<Circle2D>(area_rotated.exterior_curves.front()).origin().y(), 2);
