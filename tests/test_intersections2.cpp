...
TEST(INTERSECTIONS2, LINE_LINE) {
  Line2D line0(Point2D(1.3, 1.4), Vector2D(0, 0.7));
  Line2D line1(Point2D(-1.5, -1.7), Vector2D(1.6, 0));

  { // parallel, non-identical
  auto points = intersections2(line0, line0 + Vector2D(0.1, 0.2));
  ASSERT_EQ(points.size(), 0);
  }

  { // identical
  auto points = intersections2(line0, line1);
  ...
  { // intersecting
  auto points = intersections2(line0, line0);
  ASSERT_EQ(points.size(), 1);
  ...
  double angle = 0.31;
  auto points = intersections2(rotate(angle, line0), rotate(angle, line1));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<Line2D, Line2D, Point2D>>(points[0]);
...
TEST(INTERSECTIONS2, RAY_LINE) {
  Ray2D ray(Point2D(1.3, 1.4), Vector2D(0, 0.7));
  Line2D line(Point2D(-1.5, -1.7), Vector2D(-1.6, 0));

  { // parallel, non-intersecting
  auto points = intersections2(ray, Line2D(ray.point() + Vector2D(0.1, 0.2), ray.direction()));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(ray, Line2D(ray.point() - Vector2D(0.1, 0.2), ray.direction()));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(Line2D(ray.point() + Vector2D(0.1, 0.2), ray.direction()), ray);
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(Line2D(ray.point() - Vector2D(0.1, 0.2), ray.direction()), ray);
  ASSERT_EQ(points.size(), 0);
  ...
  auto points = intersections2(ray, Line2D(ray.point(), ray.direction()));
  ASSERT_EQ(points.size(), 1);
  ...
  auto points = intersections2(ray, line);
  ASSERT_EQ(points.size(), 0);
  }

  { // non-parallel, intersecting
  auto points = intersections2(Ray2D(ray.point(), -ray.direction()), line);
  ...
  { // intersecting rotated
  double angle = 0.31;
  auto points = intersections2(rotate(angle, Ray2D(ray.point(), -ray.direction())), rotate(angle, line));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<Ray2D, Line2D, Point2D>>(points[0]);
  Point2D result = rotate(-angle, intersection.result);
  ASSERT_DOUBLE_EQ(result.x(), 1.3);
...
TEST(INTERSECTIONS2, RAY_RAY) {
  {// parallel, non-intersecting
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(1, 1)), Ray2D(Point2D(1, 0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(Ray2D(Point2D(0, 0), -Vector2D(1, 1)), Ray2D(Point2D(1, 0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  ...
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(1, 1)), Ray2D(Point2D(1, 1), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 1);
  ...
  {// non-parallel, non-intersecting
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(1, 0.002)), Ray2D(Point2D(1, 0), Vector2D(1, 0.001)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(1, 0.002)), Ray2D(Point2D(1, 0), Vector2D(1, -0.001)));
  ASSERT_EQ(points.size(), 0);
  ...
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(-1, 0.001)), Ray2D(Point2D(1, 0), Vector2D(-1, 0.002)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(-1, 0.001)), Ray2D(Point2D(1, 0), Vector2D(-1, -0.002)));
  ASSERT_EQ(points.size(), 0);
  ...
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(1, 0.001)), Ray2D(Point2D(1, 0), Vector2D(-1, -0.001)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(1, -0.001)), Ray2D(Point2D(1, 0), Vector2D(-1, 0.001)));
  ASSERT_EQ(points.size(), 0);
  ...
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(1, 1)), Ray2D(Point2D(1, 0), Vector2D(1, 2)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<Ray2D, Ray2D, Point2D>>(points[0]);
  ...
  auto points = intersections2(Ray2D(Point2D(0, 0), Vector2D(-1, 2)), Ray2D(Point2D(1, 0), Vector2D(-1, 1)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<Ray2D, Ray2D, Point2D>>(points[0]);
...
TEST(INTERSECTIONS2, LINESEGMENT_LINE) {
  {// non-intersecting
  auto points = intersections2(LineSegment2D(Point2D(0.1, 0), Point2D(1.1, 1)),
                               Line2D(Point2D(0, 0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(0.1, 0), Point2D(1, -1)),
                               Line2D(Point2D(0, 0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(0, 0.1), Point2D(1, 1.1)),
                               Line2D(Point2D(0, 0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(0, 0.1), Point2D(1, 1)),
                               Line2D(Point2D(0, 0), Vector2D(-1, 1)));
  ASSERT_EQ(points.size(), 0);
  }

  { // segment completely on line
  auto points = intersections2(LineSegment2D(Point2D(-1.1, -1.1), Point2D(3.5, 3.5)),
                               Line2D(Point2D(0, 0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 1);
  ...
  auto points = intersections2(LineSegment2D(Point2D(3.5, 3.5), Point2D(-1.1, -1.1)),
                               Line2D(Point2D(0, 0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<LineSegment2D, Line2D, LineSegment2D>>(points[0]);
  ...
  auto points = intersections2(LineSegment2D(Point2D(4, 6), Point2D(0, 1)),
  ...
  auto points = intersections2(LineSegment2D(Point2D(0, 1), Point2D(4, 6)),
                               Line2D(Point2D(0, 0), Vector2D(2, 3)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<LineSegment2D, Line2D, Point2D>>(points[0]);
  ASSERT_DOUBLE_EQ(intersection.result.x(), 4);
  ...
  { // Clean intersections
  auto points = intersections2(LineSegment2D(Point2D(0, 6), Point2D(20, 6)),
                               Line2D(Point2D(0, 0), Vector2D(2, 3)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<LineSegment2D, Line2D, Point2D>>(points[0]);
  ASSERT_DOUBLE_EQ(intersection.result.x(), 4);
  ...
  auto points = intersections2(LineSegment2D(Point2D(2, 6), Point2D(2, 0)),
                               Line2D(Point2D(0, 0), Vector2D(2, 3)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<LineSegment2D, Line2D, Point2D>>(points[0]);
  ASSERT_DOUBLE_EQ(intersection.result.x(), 2);
...
TEST(INTERSECTIONS2, LINESEGMENT_RAY) {
  ...
  auto points = intersections2(LineSegment2D(Point2D(1, 2.5), Point2D(-0.5, -1.5)),
                               Ray2D(Point2D(0.5, 1.0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(-0.5, -1.5), Point2D(1, 2.5)),
                               Ray2D(Point2D(0.5, 1.0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(1, -2), Point2D(0, 2)),
                               Ray2D(Point2D(0.5, 1.0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(0, 2), Point2D(1, -2)),
                               Ray2D(Point2D(0.5, 1.0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  }

  { // segment completely on ray baseline
  auto points = intersections2(LineSegment2D(Point2D(-1, -1), Point2D(-0.5, -0.5)),
                               Ray2D(Point2D(0, 0), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(-1, -1), Point2D(-0.5, -0.5)),
  ...
  auto points = intersections2(LineSegment2D(Point2D(-0.5, -0.5), Point2D(-1, -1)),
                               Ray2D(Point2D(-0.5, -0.5), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<LineSegment2D, Ray2D, Point2D>>(points[0]);
  ASSERT_DOUBLE_EQ(intersection.result.x(), -0.5);
  ...
  auto points = intersections2(LineSegment2D(Point2D(-1, -1), Point2D(2.5, 2.5)),
                               Ray2D(Point2D(-0.5, -0.5), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 1);
  ...
  auto points = intersections2(LineSegment2D(Point2D(2.5, 2.5), Point2D(-1, -1)),
                               Ray2D(Point2D(-0.5, -0.5), Vector2D(1, 1)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<LineSegment2D, Ray2D, LineSegment2D>>(points[0]);
...
TEST(INTERSECTIONS2, LINESEGMENT_LINESEGMENT) {
  {// non-intersecting
  auto points = intersections2(LineSegment2D(Point2D(-1, 1.5), Point2D(1, 1.5)),
  ...
  auto points = intersections2(LineSegment2D(Point2D(1.2, 1), Point2D(2, 1.7)),
                               LineSegment2D(Point2D(-1, 1.5), Point2D(1, 1.5)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(2, 1.7), Point2D(1.2, 1)),
                               LineSegment2D(Point2D(-1, 1.5), Point2D(1, 1.5)));
  ASSERT_EQ(points.size(), 0);
  }

  {// non-intersecting, segments on same baseline
  auto points = intersections2(LineSegment2D(Point2D(1, 1), Point2D(3.5, 3.5)),
                               LineSegment2D(Point2D(-1, -1), Point2D(0, 0)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(3.5, 3.5), Point2D(1, 1)),
                               LineSegment2D(Point2D(-1, -1), Point2D(0, 0)));
  ASSERT_EQ(points.size(), 0);
  } {
  auto points = intersections2(LineSegment2D(Point2D(1, 1), Point2D(3.5, 3.5)),
  ...
  auto points = intersections2(LineSegment2D(Point2D(0.3, 0.7), Point2D(5, 1)),
  ...
  auto points = intersections2(LineSegment2D(Point2D(5, 1), Point2D(0.3, 0.7)),
                               LineSegment2D(Point2D(0.3, 0.7), Point2D(1, 5)));
  ASSERT_EQ(points.size(), 1);
  auto intersection = boost::get<Intersection<LineSegment2D, LineSegment2D, Point2D>>(points[0]);
  ASSERT_DOUBLE_EQ(intersection.result.x(), 0.3);
...
TEST(INTERSECTIONS2, POLYGON_LINE) {
  Polygon2D polygon(Triangle2D(Point2D(0, 0), Point2D(2, 0), Point2D(1, 1.5)));
  { // non-intersecting
  auto points = intersections2(polygon,
...
TEST(INTERSECTIONS2, POLYGON_RAY) {
  Polygon2D polygon(Triangle2D(Point2D(0, 0), Point2D(2, 0), Point2D(1, 1.5)));
  { // non-intersecting
  auto points = intersections2(polygon,
...
TEST(INTERSECTIONS2, POLYGON_LINESEGMENT) {
  Polygon2D polygon(Triangle2D(Point2D(0, 0), Point2D(2, 0), Point2D(1, 1.5)));
  { // non-intersecting
  auto points = intersections2(polygon,
...
TEST(INTERSECTIONS2, POLYGON_POLYGON) {
  Polygon2D polygon(Triangle2D(Point2D(0, 0), Point2D(2, 0), Point2D(1, 1.5)));
  { // non-intersecting
  auto points = intersections2(polygon,
